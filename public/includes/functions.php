<?php
	...
	function get_emp_by_district($district){
		global $conn;
		$query = "select * 
	...
	function get_all_issue1_for_catg($cat_id,$sub_cat_id,$cat_detail_id){
		global $conn;
		$query = "select * 
				from issue_master1 
				where cat_id='".$cat_id."' ";
	...
	function get_all_postedin($order){
		global $conn;
		$query = "select * from postedin_master order by ". $order ;
		$Postin=mysql_query($query,$conn);
		confirm_query($Postin);
		return $Postin;
	}
	
	function get_postedin_by_id($postedin_id){
		global $conn;
		$query = "select * from postedin_master where postedin_id='".$postedin_id."' ";
		$Postin=mysql_query($query,$conn);
		confirm_query($Postin);
		return $Postin;
	}
