...
<?php 
	...
	$designation = ucwords(trim(mysql_prep($_GET['designation'])));
	$district = ucwords(trim(mysql_prep($_GET['district'])));
	$dob_day = mysql_prep($_GET['dob_day']);
	$dob_month = mysql_prep($_GET['dob_month']);
	$dob_year = mysql_prep($_GET['dob_year']);
	$dob=$dob_year."-".$dob_month."-".$dob_day;
	$age = ucwords(trim(mysql_prep($_GET['age'])));
	$gender = mysql_prep($_GET['gender']);
	$email_id = ucwords(trim(mysql_prep($_GET['email_id'])));
	$phone_no = ucwords(trim(mysql_prep($_GET['phone_no'])));
	$office_no = ucwords(trim(mysql_prep($_GET['office_no'])));
	$mobile_no = ucwords(trim(mysql_prep($_GET['mobile_no'])));
	$postedin = mysql_prep($_GET['postedin']);
	
	$query = "update employee_master set 
			designation='".$designation."', district='".$district."', dob='".$dob."', age='".$age."', gender='".$gender."', 
			email_id='".$email_id."', phone_no='".$phone_no."', office_no='".$office_no."', mobile_no='".$mobile_no."', 
			postedin='".$postedin."'
			where  empid='".$empid."'";
